...
`performance.py` + 테이블 `live_performance`(포트폴리오 일별).

| 함수 | 내용 |
|------|------|
| `snapshot()` | 일별 평가액·일일수익률·누적수익률 + 벤치마크(KOSPI=KS11) 일수익률·액티브 저장(멱등) |
| `tracking_error()` | 액티브 일수익 표준편차 × √252(연율화) + 정보비율 + 일평균 액티브 |
| `performance_summary()` | 누적수익·벤치마크 누적·액티브수익·MDD·추적오차·정보비율 |

- `paper_trade` 스테이지가 매매 후 **매일 자동 스냅샷** → 스테이지 메시지에
  `평가/누적%/TE` 표기.
- 벤치마크는 `macro_prices`의 KS11(KOSPI). 추적오차는 액티브 일수익 2일 이상부터 산출.

테스트 5건(첫날 무수익률·일일/누적 계산·추적오차·요약·멱등). **전체 151 passed.**
